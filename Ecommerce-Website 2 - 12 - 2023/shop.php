<?php
    ...
    if (isset($_GET['sort']) || isset($_GET['category']) || isset($_GET['brand']) || isset($_GET['size']) || isset($_GET['color'])) {
        $query = filterSort();
...
//         echo "Id: " . $row["ProductId"] . " - Name: " . $row["ProductBrand"] . "<br>";
//     }
// } else {
...
<!-- 
                ...
                <option value="default" selected disabled hidden>Size</option>
                    <?php
                        // $uniqueSizes = array_unique(array_column($result1, 'sName'));
        ...
        <div class="pro-container">
            <?php
                ...
                if ($result) {
                    foreach ($result as $product_char) {
                    
                        echo '<div class="pro">';
                        echo '<a href="sproduct.php?buttonClicked=' . $product_char["ProductId"] . '">';
                        echo '<img src="data:image/jpeg;base64,' . base64_encode($product_char["Image"]) . '" width="100%" alt="">';
                        echo '</a>';
                        echo '<div class="des">';
                        echo '<span>' . $product_char["ProductBrand"] . '</span>';
                        echo '<h5>'. substr($product_char["Description"], 0, 48)  . "..." .'</h5>';
                        for ($y = 1; $y <= $product_char["Rating"]; $y++) {
                            echo '<i class="fas fa-star"></i>';
                        }
                        echo '<h4>$ ' . $product_char["Price"] . '</h4>';
                        echo '</div>';
                        echo '<a href="cart.php?product_id=' . $product_char["ProductId"] . '&buttonClicked=' . $product_char["ProductId"] . '"><i class="fa-solid fa-heart"></i></a>';
                        echo '</div>';
                    }
                } else {
                    echo "No results found.";
